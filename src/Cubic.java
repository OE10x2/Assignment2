...
public class Cubic extends Function implements Calculations, Drawable{
    ...
    public void draw(Canvas canvas){
        ...
        while (loop <= domain2){
            highest = Math.max(highest, val(loop));
            lowest = Math.min(lowest, val(loop));
            loop = Math.round((loop + delta) * 100.0) / 100.0;
        }
        ...
        while (loop <= domain2){
            ...
            double endX = ratioX * (loop - adjustX) + screenX/2;
            double endY = ratioY * (-val(loop) - adjustY) + screenY/2;
        ...
        System.out.println("RANGE: " + rangeLabel + " " + lowest + " " + highest);
        gc.setLineWidth(2.0);
        ...
        for (int i = 1; i <= 9; i++){
            ...
            if (lowest >= 0){
                gc.fillText(labelX, curLoopX, screenY-20);
                gc.fillText("|", curLoopX, screenY-5);
            }else if (highest <= 0){
                gc.fillText(labelX, curLoopX, 20);
                gc.fillText("|", curLoopX, 5);
            }else{
            ...
            double curLoopY = screenY / 10 * i;
            String labelY;
            String left = "- " + Math.round(-curY * 10.0) / 10.0;
            String right = Math.round(-curY * 10.0) / 10.0 + " -";
            if (lowest >= 0){
                gc.setTextAlign(TextAlignment.LEFT);
                labelY = left;
                gc.fillText(labelY, 0, curLoopY);
            }else if (highest <= 0){
                gc.setTextAlign(TextAlignment.RIGHT);
                labelY = right;
                gc.fillText(labelY, screenX, curLoopY);
            }else{
                //Y-AXIS IS BEING DISPLAYED
                double temp = screenY * lowest / (highest - lowest);
                gc.strokeLine(temp, 0, temp, screenY);
                if (temp / screenX > 0.5){
                    gc.setTextAlign(TextAlignment.RIGHT);
                    labelY = right;
                }else{
                ...
                gc.fillText(labelY, temp, curLoopY);
            }
        }
    }
}